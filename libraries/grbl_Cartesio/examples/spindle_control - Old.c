...
{    
  ...
  #ifdef VARIABLE_SPINDLE
    SPINDLE_PWM_DDR |= (1<<SPINDLE_PWM_BIT); // Configure as PWM output pin.
     //Modified by MBO to fit to ESC (brushless spindle) 
   TCCRA_REGISTER = (1<<COMB_BIT) | (1<<WAVE1_REGISTER) | (1<<WAVE0_REGISTER);
    TCCRB_REGISTER = (TCCRB_REGISTER & 0b11111000) | 0x06; // 0x06 to set to 1/256 Prescaler->122.55Hz/ default: 0x02 to set to 1/8 Prescaler
   OCR_REGISTER = 0;//50; //start-up condition
  ...
  #ifndef USE_SPINDLE_DIR_AS_ENABLE_PIN
    SPINDLE_DIRECTION_DDR |= (1<<SPINDLE_DIRECTION_BIT); // Configure as output pin.
  #endif SPINDLE_DIRECTION_DDR |= (1<<SPINDLE_DIRECTION_BIT); // Configure as output pin.
  spindle_stop();
}
...
void spindle_stop()
{
 ...
 //Modified by MBO to fit to ESC (brushless spindle) 
 //TCCRA_REGISTER &= ~(1<<COMB_BIT); // Disable PWM. Output voltage is zero.
    OCR_REGISTER = 0; //-> 0.8ms -> ESC driven spindle stops
   #ifndef CPU_MAP_ATMEGA328P 
      SPINDLE_ENABLE_PORT &= ~(1<<SPINDLE_ENABLE_BIT); // Set pin to low.
    #endif
  #else
